...
import com.ericsson.fdp.business.route.processor.UssdHttpRequestProcessor;
...
/*@Startup
@Singleton(name = "UssdHttpServiceRoute")*/


public class UssdHttpServiceRoute  {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(UssdHttpServiceRoute.class);
	private static final String ussdHttpServlet = "servlet:///App?servletName=UssdHttpService&matchOnUriPrefix=false";
	...
	@Inject
	private UssdHttpRequestProcessor requestProcessor;
	...
	public void createRoutes() throws Exception {
		...
		for (final UssdHttpRouteEnum httpService : UssdHttpRouteEnum.values()) {
	 ...
	 */
	private RouteBuilder createHttpServiceRoute(final UssdHttpRouteEnum httpService){
		...
		case USSD_HTTP_SERVICE:
			...
			.process(requestProcessor).log("came here finally.............");
