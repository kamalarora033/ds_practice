 ...
 * 
 */
@Stateless(mappedName = "TimeToShareOfferServiceImpl")
public class TimeToShareOfferServiceImpl implements TimeToShareOfferService {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = LoggerFactory.getLogger(TimeToShareOfferServiceImpl.class);
	...
	public boolean updateMetaCache(final UpdateCacheDTO updateCacheDTO) throws FDPServiceException {
		...
		if (updateCacheDTO != null) {
			...
			if (ActionTypeEnum.DELETE.equals(updateCacheDTO.getAction())) {
				...
				if (ModuleType.TIME_TO_SHARE.equals(updateCacheDTO.getModuleType())) {
					return fdpCache.removeKey(new FDPMetaBag(updateCacheDTO
							.getCircle(), ModuleType.TIME_TO_SHARE, id));
				}
			}
	...
	public boolean initializeMetaCache(FDPCircle fdpCircle)
		...
		if (fdpCircle == null) {
			throw new FDPServiceException("Time to Share cache initialization: Circle cannot be null");
		} else {
	...
	private void initializeUpdateTTSOffer(final ShareBaseOfferDTO shareBaseOfferDTO, final FDPCircle fdpCircle) {
		...
		FDPMetaBag metaBag = new FDPMetaBag(circle, ModuleType.TIME_TO_SHARE,shareBaseOfferDTO.getOffer_id());
