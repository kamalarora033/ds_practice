...
/**
 * This class is the interceptor for the three G activation over MML.
 * 
 * @author Ericsson
 * 
 */
public class FDPThreeGMMLActivationCommandImpl extends FDPDefaultActivationCommandImpl {

	/**
	 * The string to identify if threeG is already activated.
	 */
	private static final String THREE_G_DEACTIVATED = "ARD-1";

	/**
	 * The constructor.
	 * 
	 * @param fdpCommand
	 *            the command intercepted.
	 */
	public FDPThreeGMMLActivationCommandImpl(final FDPCommand fdpCommand) {
		super(fdpCommand);
	}
	...
	private boolean checkIfCommandNeedsExecution(final FDPRequest fdpRequest) throws ExecutionFailedException {
		...
		if (ThreeGActivationCommandMode.QUERY_AND_UPDATE_ALL.equals(threeGActivationCommandMode)) {
			...
			if (responseValue.contains(THREE_G_DEACTIVATED)) {
				needsExecution = true;
			}
		}
