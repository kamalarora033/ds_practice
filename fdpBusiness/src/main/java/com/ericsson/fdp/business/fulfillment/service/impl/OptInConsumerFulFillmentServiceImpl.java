...
@Stateless
public class OptInConsumerFulFillmentServiceImpl extends AbstractFDPFulFillmentServiceImpl {
	...
	protected FDPResponse executeService(FDPRequest fdpRequest, Object... additionalInformations) throws ExecutionFailedException {
		...
		FDPLogger.debug(logger, getClass(), "execute OptInConsumerFulFillmentServiceImpl", LoggerUtil.getRequestAppender(fdpRequest)
        ...
        ServiceProvisioningRule sp = RequestUtil.getServiceProvisioningById(fdpRequest, 
        		ServiceProvDataConverterUtil.getKeyForServiceProvMetaBag(Long.valueOf(product.getProductId().toString()), FDPServiceProvType.PRODUCT, FDPServiceProvSubType.PRODUCT_BUY));
		...
		if(isRequestValid(fdpRequest, fdpResponse)){
			if(doOptIn(fdpRequest, fdpResponse)){
		        fdpResponse.setExecutionStatus(Status.SUCCESS);
		        fdpResponse.setFulfillmentResponse("Optin request successful");
			}
		}
		FDPLogger.debug(logger, getClass(), "execute OptInConsumerFulFillmentServiceImpl", LoggerUtil.getRequestAppender(fdpRequest)
	...
	/**
	 * This method hits the RS for optin(Adhoc to Recurring)
	 * @param fdpRequest
	 * @param fdpResponse
	 * @throws ExecutionFailedException 
	 */
	private Boolean doOptIn(FDPRequest fdpRequest, FDPMetadataResponseImpl fdpResponse) throws ExecutionFailedException {
		return Status.SUCCESS.equals(CommandUtil.executeCommand(fdpRequest, Command.SPR, true));
	}

	/**
	 * This method validates if Optin request is a valid request
	...
	private boolean isRequestValid(FDPRequest fdpRequest, FDPMetadataResponseImpl fdpResponse) throws ExecutionFailedException {
		return (isAdhoc(fdpRequest, fdpResponse) && isRecurringProvAvailable(fdpRequest, fdpResponse) && isProductSubscribedByUser(fdpRequest, fdpResponse));
	}
	 ...
	 * @return
	 */
	private boolean isAdhoc(FDPRequest fdpRequest, FDPMetadataResponseImpl fdpResponse) {
		boolean isAdhoc = false;
		FulfillmentRequestImpl fulfillmentRequestImpl = (FulfillmentRequestImpl) fdpRequest;
		String requestString = fulfillmentRequestImpl.getRequestString();
		if(null != requestString && requestString.contains(FDPConstant.NORMAL_PRODUCT_PREFIX)){
			isAdhoc = true;
		}else{
			fdpResponse.setFulfillmentResponse("Invalid Request. Optin valid only for Adhoc products");
		}
		return isAdhoc;
	}

	/**
	 * This method checks if the product has SP for recurring 
	 * @param fdpRequest
	 * @param fdpResponse
	 * @return
	 * @throws ExecutionFailedException
	 */
	private boolean isRecurringProvAvailable(FDPRequest fdpRequest, FDPMetadataResponseImpl fdpResponse) throws ExecutionFailedException {
		...
		final FDPCacheable serviceProvisioning = RequestUtil.getServiceProvisioningById(fdpRequest,
				ServiceProvDataConverterUtil.getKeyForServiceProvMetaBag(getProductId(fdpRequest), FDPServiceProvType.PRODUCT,
						FDPServiceProvSubType.PRODUCT_BUY_RECURRING));
		if (serviceProvisioning != null){
			isRecurring = true;
		}else {
			fdpResponse.setFulfillmentResponse("Invalid Request. No Recurring provisioning available for this request");
		}
		return isRecurring;
	}
	...
	private boolean isProductSubscribedByUser(FDPRequest fdpRequest, FDPMetadataResponseImpl fdpResponse) throws ExecutionFailedException {
		...
		if (cachedCommand instanceof FDPCommand) {
			...
			if(Status.SUCCESS.equals(status)){
				...
				FDPLogger.info(logger, getClass(), " OptInConsumerFulFillmentServiceImpl", "isProductSubscribedByUser listSize :"
				+ isProductSubscribed);
	...
	private Set<String> getSubscribedProducts(final FDPCommand executedCommand) {
		...
		FDPLogger.info(logger, getClass(), " OptInConsumerFulFillmentServiceImpl", "getSubscribedProducts listSize :"
	...
	private Long getProductId(final FDPRequest fdpRequest) throws ExecutionFailedException{
		...
		if(null != cachedProduct && cachedProduct instanceof Product){
			...
			FDPLogger.info(logger, getClass(), " OptInConsumerFulFillmentServiceImpl", "getProductId() Product ID :"
